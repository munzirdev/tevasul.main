...
serve(async (req) => {
  ...
  try {
    ...
    // Check if this is a Telegram webhook request (no authorization header)
    const hasAuthHeader = req.headers.get('authorization') || req.headers.get('x-client-info') || req.headers.get('apikey')
    
    if (!hasAuthHeader) {
      console.log('🔔 Telegram webhook request detected (no auth header)')
      // For Telegram webhook requests, we don't need authorization
    } else {
      console.log('🔔 Supabase function request detected (with auth header)')
    }
