-- Apply Residence Services Database Updates
-- This script updates the database to support new residence services

-- 1. Update service_requests constraint to include new service types
...
-- 2. Create residence_requests table
...
-- 4. Enable RLS on residence_requests table
ALTER TABLE public.residence_requests ENABLE ROW LEVEL SECURITY;

-- 5. Create policies for residence_requests table
DO $$
BEGIN
...
-- 6. Create function to handle updated_at trigger for residence_requests
...
-- 7. Create trigger for residence_requests updated_at
DO $$
BEGIN
...
-- Check service_requests constraint
SELECT 
...
-- Check residence_requests table
SELECT 
    table_name,
    column_name,
    data_type,
...
AND table_name = 'residence_requests'
...
SELECT 
    ...
    with_check
FROM pg_policies 
WHERE tablename = 'residence_requests'
ORDER BY policyname;

SELECT '=== UPDATES COMPLETED SUCCESSFULLY ===' as info;
